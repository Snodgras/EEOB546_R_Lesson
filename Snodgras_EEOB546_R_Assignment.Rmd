...
#SNPbyChr
filteredSNPbyChr <- ggplot(data = filtered.snp.position) + geom_bar(mapping = aes(x = as.character(Chromosome))) #without multiple and unknown positioned SNPs
filteredSNPbyChr
```
...
tapply(Filtered.TRIPS$Genotype, Filtered.TRIPS$SNP_ID, unique) %>% lengths() -> temp #Determines the length of each element, i.e. how many unique genotypes there are by SNP_ID, this doesn't include NA so length of 1 = all individuals have the same genotype, 2 = there are 2 possible genotypes, 3 = there are three possible genotypes (both homozygous conditions and heterozygous)
...
CollapseRegrouped <- select(Regrouped,-"Sample_ID", -"JG_OTU", -"Genotype", -"homozygous") #removes these columns so they don't count accidentally in distinct 
CollapseRegrouped <- distinct(CollapseRegrouped) #saves only unique rows --> collapses the data down into a single row for each group and each snp

#SNPbyID <- ggplot(data = Regrouped) + geom_bar(mapping = aes(x = SNP_ID, color = TrueSNP, fill=TrueSNP, position="fill"))
#SNPbyID

SNPbyGroup <- ggplot(data = CollapseRegrouped) + geom_bar(mapping = aes(x = Group, color = TrueSNP, fill = TrueSNP, position="fill"))+ theme(axis.text.x = element_text(angle = 90, hjust = 1))
SNPbyGroup
```
...
all.snps.collapsed <- distinct(all.snps.bygroup, Genotype, Group, SNP_ID)

all.snps.collapsed <- count_(all.snps.bygroup, var = c("Group", "SNP_ID", "Genotype")) #creates column "n" that has counts of number of rows with unique values in each of the columns combined in var
SNPHom.plot1 <- ggplot(data = all.snps.collapsed) + geom_jitter(mapping = aes(x=Genotype, y=n,color=Group, alpha=0.5))
SNPHom.plot1 #shows genotype on x axis with counts for unique Group/SNP_ID/Genotype combos on y axis, colored by group
SNPHom.plot2 <- ggplot(data = all.snps.collapsed) + geom_bar(mapping = aes(x = Group, color = Genotype, fill = Genotype)) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
SNPHom.plot2 #counts of distinct Group by Genotype by SNP_ID, shows group on x axis and counts of each group on y
```
